...
# Set paths
$sourceDir = Join-Path -Path $repoRoot -ChildPath "functions"
$deploymentDir = Join-Path -Path $repoRoot -ChildPath "deployment"
$outputFile = Join-Path -Path $deploymentDir -ChildPath "function-package.zip"

# Verify source directory exists
if (-not (Test-Path $sourceDir)) {
    Write-Host "ERROR: Functions directory not found: $sourceDir" -ForegroundColor Red
    exit 1
}
...
try {
    ...
    Write-Host "  WARNING: Could not list package contents" -ForegroundColor Yellow
}

Write-Host "`nIMPORTANT: Commit this file to GitHub:" -ForegroundColor Yellow
Write-Host "   git add deployment/function-package.zip" -ForegroundColor White
Write-Host "   git commit -m 'Update deployment package v2.3.0'" -ForegroundColor White
