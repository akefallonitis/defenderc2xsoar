...
.DESCRIPTION
    Configures the MINIMUM required permissions for DefenderXDR C2 v3.0.0:
    - Microsoft Graph API (11 permissions for Entra ID, MDO, Intune, MCAS)
    - Microsoft Defender for Endpoint API (3 permissions)
    
    Total: 14 application permissions (least privilege)
...
# Define required permissions (MINIMUM for v3.0.0)
$requiredPermissions = @{
    # Microsoft Graph API
    "00000003-0000-0000-c000-000000000000" = @(
        ...
        @{ Id = "c7fbd983-d9aa-4fa7-84b8-17382c103bc4"; Name = "ThreatIndicators.ReadWrite.OwnedBy"; Type = "Role" } # MDO - Manage indicators
...
Write-Host "  Expected: 14 permissions" -ForegroundColor Cyan

if ($totalPermissions -eq 14) {
    Write-Host "`n✅ VERIFICATION PASSED - 14 permissions configured correctly!`n" -ForegroundColor Green
} else {
    Write-Host "`n⚠️  WARNING: Expected 14 permissions but found $totalPermissions" -ForegroundColor Yellow
}
...
Write-Host "  Microsoft Graph API          : 11 permissions" -ForegroundColor Gray
Write-Host "  Microsoft Defender Endpoint  : 3 permissions" -ForegroundColor Gray
Write-Host "  ────────────────────────────────────────" -ForegroundColor DarkGray
Write-Host "  TOTAL                        : 14 permissions" -ForegroundColor Green
Write-Host ""
